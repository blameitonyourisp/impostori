...
class BitBuffer {
    ...
    write(int, size, offset = 0, strict = false) {
    ...
    writeString(string, offset = 0, strict = false) {
    ...
    read(size, offset = 0, strict = false) {
        offset = strict ? offset : this.#clampOffset(offset)
        size = strict ? size : this.#clampSize(size, offset)
    ...
    readString(size, offset = 0, strict = false) {
        let string = ""
    ...
    async hash(secret = "") { // up to 128 utf-16 char string
        ...
        let uint24 = view[0] << 16 | view[1] << 8 | view[2]
    ...
    static async from(string, secret = "") {
        ...
        for (const match of string.match(regex)) {
            ...
            buffer.writeSequential(uint24, 24)
        }
    ...
    // add specific error texts
    static get #SizeError() {
        return class BitBufferSizeError extends RangeError {
    ...
    static get #RangeError() {
        return class BitBufferRangeError extends RangeError {
    ...
    static get #StringError() {
        return class BitBufferStringError extends TypeError {
            constructor() {
                super("An integer size of ${} bits was requ")
            }
        }
