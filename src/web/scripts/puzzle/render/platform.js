 ...
 * @returns {Container}
 */
const getCellPlatform = (cell, data, {
    isGrid = true,
    scale = 1
} = {}) => {
    ...
    const selector = data.permutedIndexArray[cell.index]
    const platformKey = 1 << cell.box & parseInt("100110", 2)
        ? `platform-vertical-${selector % 6 + 1}-cw`
        : `platform-horizontal-${selector % 6 + 1}-cw`
    const platform = new Sprite(data.spritesheet[platformKey])
    // const platform = new Sprite(data.spritesheet["platform-base-w"])
    container.addChild(platform)

    const tiles = getCellTiles(cell, data, isGrid)
    for (const [index, tile] of tiles.entries()) {
        tile.setTransform(...TILE_LOCATIONS[index])
        container.addChild(tile)
    }

    const { index, row, column, box } = data.selectedCell
    const centerDotKey = cell.index === index ? "dot-center-6-x"
        ...
        : "dot-center-1"
    const centerDot = new Sprite(data.spritesheet[centerDotKey])
    ...
    if (isGrid) {
        ...
        for (const index of cell.adjacentIndexes.all) {
            ...
            switch (cell.index - index) {
                case 5:
                    connection = new Sprite(data.spritesheet["connection-1-c"])
                    connection.setTransform(...CONNECTION_LOCATIONS[0])
                    break
                case 6:
                    connection = new Sprite(data.spritesheet["connection-2-c"])
                    connection.setTransform(...CONNECTION_LOCATIONS[1])
                    break
                default:
                    connection = new Sprite(data.spritesheet["connection-3-c"])
